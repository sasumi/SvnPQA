	...
	<form action="<?php echo $this->getUrl($this->getController().'/update', array($pk=>$model_instance->$pk));?>" class="frm" rel="async" method="post">
		...
		<table class="frm-tbl">
			...
			<tbody>
				...
				<tr>
					<td class="col-label">生产类型</td>
					<td>
						<?php foreach($defines['produce_type']['options'] as $k=>$v):?>
						<label>
							<input type="radio" name="produce_type" value="<?php echo $k;?>" <?php echo ($model_instance->produce_type == $k || !$model_instance->id && $k == 0) ? 'checked="checked"' : '';?>>
							<?php echo $v;?>
						</label>
						<?php endforeach;?>
					</td>
